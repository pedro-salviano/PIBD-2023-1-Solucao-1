...
WHERE o.id IS NULL;
*/
CREATE OR REPLACE FUNCTION getGradSemOrientador() 
RETURNS TABLE (
    professor_isf_id INT,
    Poca_File TEXT, 
    Edital_Selecao_File TEXT, 
    Resultado_Selecao_File TEXT, 
    Termo_Turma_Compromisso_File TEXT, 
    Vinculo_File TEXT
)
LANGUAGE plpgsql
AS $$
BEGIN
    ...
    SELECT 
        v.professor_isf_id, 
        v.Poca_File, 
        v.Edital_Selecao_File, 
        v.Resultado_Selecao_File, 
        v.Termo_Turma_Compromisso_File, 
        v.Vinculo_File
    FROM vGradSemOrientador v;
END;
$$;
/*--testando entrada de caso sem orientador
SELECT * FROM getGradSemOrientador();
*/
